...
@section('content')
    @include('layouts.navbars.auth.topnav', ['title' => 'Timetables'])
                        ...
                        <tbody>
                            @forelse($instances as $instance)
                                @if (auth()->user()->role == 'DEAN')
                                    @if(count($instance->schools()->where('school_id', auth()->user()->deanOf->school_id)) > 0)
                                        <tr>
                                            <td>
                                            ...
                                            <td class="align-middle text-center text-sm">
                                                ...
                                                @elseif($instance->status == 'Generating')
                                                    <span class="badge badge-sm bg-gradient-info">Generating</span>
                                                @elseif($instance->status == 'queued')
                                                    <span class="badge badge-sm bg-gradient-danger">Queued</span>
                                                @endif
                                            </td>
                                            ...
                                            <td class="align-middle">
                                                @if($instance->status == 'Done')
                                                    <a href="/timetables/{{$instance->timetable_instance_id}}" class="btn btn-success btn-xs font-weight-bold text-xs mb-0" data-original-title="View Timetable">
                                                        View
                                                    </a>
                                                    @if (auth()->user()->role == 'ADMIN')
                                                        <a class="btn btn-primary btn-xs font-weight-bold text-xs mb-0 delete" data-id="{{$instance->timetable_instance_id}}">
                                                            Delete
                                                        </a>
                                                    @endif
                                                @elseif($instance->status == 'Generating')
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center">
                                        <p class="text-xs font-weight-bold mb-0">No schools found! 
                                            @if($schools->count() > 0)
                                                <a href="/generate">Generate New Timetable</a>
                                            @endif
                                        </p>
                                    </td>
                                </tr>
